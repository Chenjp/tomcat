...
public interface WebResourceSet extends Lifecycle {
    ...
    /**
     * Fully writes to a resource at the requested path using the provided InputStream, creates a new resource if
     * absent.
     * <p>
     ...
     *                      must start with '/'.
     * @param is        The InputStream that will provide the content for the resource.
     ...
     *
     * @return <code>true</code> if and only if the specified content from the source input stream is written to the path
     *             resource successfully
     */
    boolean write(String path, InputStream is, boolean overwrite);

    /**
     * Partially writes up to {@code count} bytes to a resource at the requested path using the provided InputStream,
     * creates a new resource if absent. Underlying overwrite mode is {@code true}.
     * <p>
